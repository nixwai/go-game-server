...
func (s *AuthService) Login(ctx context.Context, username, password string) (LoginResult, error) {
	...
	if err != nil || user.Status != model.StatusActive || !s.hasher.Compare(password, user.PasswordHash) {
		return LoginResult{}, response.NewError(401, response.CodeAuthFailed, "invalid username or password", nil)
	}
...
func validateUsername(username string) error {
	...
	if len(username) < 3 || len(username) > 64 || !usernamePattern.MatchString(username) {
		return response.NewError(400, response.CodeValidation, "invalid username", nil)
	}
	return nil
}
...
func validatePassword(password string) error {
	if len(password) < 8 || len(password) > 128 {
		return response.NewError(400, response.CodeValidation, "invalid password", nil)
	}
