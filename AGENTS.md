...
- 围棋坐标统一使用 `[x,y]`：`x` 表示从左到右的列，`y` 表示从上到下的行；棋盘布局使用 `layout[y][x]`，例如 `[1,2]` 对应 `layout[2][1]`。
...
- 不为了复用而提前创建无业务价值的抽象层。
- 校验逻辑分三层：
  - DTO 标签校验：能用 `binding` 标签表达的规则（如 `required`、`oneof`、`min`、`max`）声明在 DTO 结构体上，由 Handler 绑定时自动执行；
  - 领域校验函数：标签无法表达的纯输入校验（如 trim 后长度、URL 格式解析、棋盘矩阵结构）放在模块内 `validate.go` 文件中，作为包内未导出函数；
  - 业务规则校验：依赖数据库状态的校验（如归属权、配额限制）作为 Service 方法，不抽离到 `validate.go`；
- 校验逻辑不跨模块复用，不创建公共校验包。
